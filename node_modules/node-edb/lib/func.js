...
let fs = require('fs')
 class Func {
  ...
  checkDataType(table_name,data){
    let tableData = JSON.parse(fs.readFileSync(`${this.url}\\${this.db}\\${table_name}.json`))
    tableData.code = 200
  ...
  */
  select(table_name,option){
    try{
      let dataString = fs.readFileSync(`${this.url}\\${this.db}\\${table_name}.nb`).toString()
      let tableData = parsing.stringToJson(dataString)

      if(option.where){ // where filtrate
        tableData = parsing.whereFiltrate(tableData,option.where)
      }

      if(option.limit) { // limit filtrate
        tableData = parsing.limitFiltrate(tableData,option.limit)
      }

      if(option.sort) { // sort filtrate
      ...
      if(option.paging) { // paging filtrate
        const data = parsing.pagingFiltrate(tableData,option.paging)

        if(option.fields) { // fields filtrate
  ...
  */
  insert(table_name,data){
    ...
    // tableDesign.autoId ++
    fs.writeFileSync(`${this.url}\\${this.db}\\${table_name}.json`,`${JSON.stringify(tableDesign)}`)
    fs.writeFileSync(`${this.url}\\${this.db}\\${table_name}.nb`, `${JSON.stringify(data)}\n`, {'flag':'a'})
    return {
      code:200,
  ...
  update(table_name,data,option)  {
    try{
      ...
      let outData = JSON.stringify(tableData).replace(/},/g,'}\n').replace('[','').replace(']','')

      fs.writeFileSync(`${this.url}\\${this.db}\\${table_name}.nb`, `${outData}\n`, )
      return {
        code: 200,
        message: 'success',
      }
